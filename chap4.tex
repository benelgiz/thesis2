...
The model can be written in a vectorized form to accommodate larger number of features in a more compact form.
To write in the compact form, an extra feature $\bm{x}_0 = 1$ is added to the feature vector such as
...
Since now the parameters to fit for is two, $\bm{\theta}$ and $d$, three different sets of data are necessary to train, tune and evaluate.
Similar to the case of training and evaluating the model with the same data (for a given model) might lead to a biased evaluation, with an added necessity to also select (or tune) the model requires an extra set of data. Those three separate sets of data are called training, cross-validation and test sets.
...
Then, the model structure $d$ which makes $J_{cv}({\bm{\theta}}^{(d)})$ minimum is selected and its generalization is error is calculated with the test set $J_{test}({\bm{\theta}}^{(d)})$.
...
An important point is to keep that values $\mu_j$ and $s_j$ (or standard deviation in cases it is preferred instead of range $s_j$) which are calculated during training. 
...
Finally, the classifier trained is evaluated by calculating the test set error using the test set.
Those steps are given in more detail as follows:
 ...
 \begin{enumerate}
  ...
  Note that if the data set is randomly ordered, then the first 70\% can be used for training and the rest for test, but if the data is not randomly ordered (data set is dependent on time or any kind of pattern in between the data set) 70\% - 30\% should be selected randomly.
  ...
  \item Calculate the test set error by using test set (30\% of whole data) with the parameters learnt by using the training set (70\% of the whole data).\\
...
They usually require the cost function, its gradient and Hessian approximation in order to calculate the best $\bm{\theta}$ values.

\textbf{Gradient Descent}

The gradient descent will be presented for one feature problem due to its simplicity. 
...
But for linear regression, it is shown that the cost function shown above is convex, meaning that it has no local minima but only one global minima. An example for cost function converging to different solutions depending on initialization is given in Fig.\ref{fig:localOrGlobalMinimaGD} (slightly different choice of $\theta_{init}$ might converge to the local minima, which is smaller compared to surrounding regions, but does not converge to the smallest $J(\bm{\theta})$.
...
Fig.~\ref{fig:underOverFit} shows three model examples plotted with the data that they are trained with. From left to write, those three regression problem models are underfit, satisfactory or overfit. 
...
In regularization, a term is added to the cost function in order to penalize parameters $\vec{\bm{\theta}}$ for being too large (i.e to make $\vec{\bm{\theta}}$ as small as possible), except ${\theta}_0$ (is not penalized by convention). Cost function for linear regression with the added term for regularization is given as
...
So it tends to decrease the values of $\theta$. But if this lambda is too big, then the model converges to  
...
Thus, the model becomes underfit for large values of $\lambda$, and it should be tuned to acquire the optimal value.
...
The training and cross validation errors are calculated in the same way as the cost function since cost function itself is a measure of error.
...
After diagnosing the problem is an over-fit or under-fit, those practices are helpful to know. 
...
Finally, for a new set of input data, the output can be predicted using the model/classifier trained. 
...
After that, $h(\theta)$ could be evaluated using optimized $\theta$ and scaled new data set $x\_new\_scaled$. 
...
In this study, Gaussian Kernel, which corresponds to an infinite dimensional feature mapping, is used to map the attributes.
...
The classifier is trained with the training set and tuned via cross validation set, and then the selected classifier is evaluated with the test set. Cross-validation set selection of Matlab uses a random selection over the data set. 
...
Also a geometric sequence of the box constraint parameter have been tried (11 values, from 1e-5 to 1e5 by a factor of 10). Increasing box constraint might decrease the number of support vectors, but also might increase training time. 
...
The training data set is separated to 10 folds. For each fold, the \emph{loss of classification} is computed for in-fold observations with a model trained on out-of-fold observations. 
